...
import { counterId, Defer, pLimitFn, until } from '../../src/utils'
import Connection from '../../src/Connection'
import { StorageNode } from '../../src/stream/StorageNode'
import { StreamOperation } from '../../src/stream'
...
describeRepeats('PubSub with multiple clients', () => {
    ...
    beforeAll(async () => {
        ...
        const nodeAddress = await mainClient.getAddress()
        await mainClient.setNode('metadata')
        await stream.addToStorageNode(nodeAddress)
        return until(async () => {
            try {
                return await mainClient.isStreamStoredInStorageNode(stream.id, nodeAddress)
            } catch (err) {
                mainClient.debug('stream not found yet %o', err)
                return false
            }
        }, 100000, 1000)
    })
